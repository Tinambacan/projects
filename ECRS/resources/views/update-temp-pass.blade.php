...
<head>
    ...
    @vite('resources/js/change-temp-pass.js')
    <script>
        (function() {
...
<body>
    <div
        ...
        <x-set-pass-nav-bar />
        <main
            class="flex-1 px-3 w-full  dark:bg-[#1E1E1E] transition-all duration-300 flex flex-col justify-center items-center mt-12">
            <div class="dark:text-white w-full">
                <div class="flex flex-col justify-center items-center animate-fadeIn">
                    ...
                    <form id="update-temp-password-form" method="POST" class="md:w-[30rem] w-full">
                        @csrf
                        <input type="hidden" id="email" name="email" value="{{ $email }}">
                        <input type="hidden" id="role" name="role" value="{{ $role }}">

                        <div
                            class="bg-white border dark:bg-[#161616] border-gray-300 dark:border-[#404040] dark:text-white text-black  rounded-md shadow-lg p-4 my-5">
                            <div class="my-6">
                                ...
                                @if ($role == 1)
                                    <div class="flex justify-center pt-5 gap-2">
                                        <button id="set-new-temp-pass-btn"
                                ...
                                @elseif ($role == 2)
                                    <div class="flex justify-center pt-5 gap-2">
                                        <button id="set-new-temp-pass-btn-ad"
                                            class="text-black rounded-lg px-5 py-2 shadow-lg border border-gray-300 dark:text-white hover:bg-gray-100 dark:hover:bg-[#1E1E1E]">
                                            Update password
                                        </button>
                                    </div>
                                @else
                                    <div class="flex justify-center pt-5 gap-2">
                                        <button id="set-new-temp-pass-btn-st"
                                            class="text-black rounded-lg px-5 py-2 shadow-lg border border-gray-300 dark:text-white hover:bg-gray-100 dark:hover:bg-[#1E1E1E]">
                                            Update password
                                        </button>
                                    </div>
